  ...
  <div class="operation-form">
    ...
    <el-form v-else-if="!advanced" label-position="top" :disabled="disabled">
      <el-form-item v-for="field in fields" :key="field.name" :label="field.label" :data-field-name="field.name" :data-field-type="field.type">
        <template #label
          >{{ field.label }} <small v-if="['files', 'paths', 'directories', 'list', 'tables'].includes(field.type) && Array.isArray(values[field.name])" class="item-count">{{ values[field.name].length }} 项</small></template
        >
        <div class="field-value">
          ...
          <el-select v-if="previous.length" :disabled="disabled" placeholder="引用前一步结果" clearable class="reference" @change="reference(field, $event)"><el-option v-for="step in previous" :key="step.id" :value="step.id" :label="step.name || step.id" /></el-select>
        </div>
      </el-form-item>
