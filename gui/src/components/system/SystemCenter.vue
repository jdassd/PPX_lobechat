...
const openFolderOfPath = (path) => {
  ...
  window.pywebview.api.system_pyOpenFile(dir)
}
...
const loadCDriveCustomRules = async () => {
  ...
  try {
    const res = await window.pywebview.api.system_listCDriveCustomRules()
    if (res?.success) {
...
const cleanCDriveFiles = async () => {
  ...
  cDriveCleanState.cleaning = true
  try {
    const res = await window.pywebview.api.system_cleanCDriveFiles({
...
const addSelectedFilesToWhitelist = async () => {
  ...
  try {
    const res = await window.pywebview.api.system_addCDriveWhitelist({
...
const pickCDriveRulePath = async () => {
  ...
  const dir = await window.pywebview.api.system_pySelectDirDialog(cDriveCleanState.ruleForm.path || '')
...
const saveCDriveRule = async () => {
  ...
  const res = await window.pywebview.api.system_saveCDriveCustomRule(payload)
  if (res?.success) {
...
const removeCDriveRule = async (row) => {
  ...
  const res = await window.pywebview.api.system_removeCDriveCustomRule({ id: row.id })
  if (res?.success) {
    cDriveCleanState.customRules = res.rules || []
