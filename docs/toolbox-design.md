# PPX 统一工具箱设计方案（2.10.0 阶段）

## 1. 定位、目标与设计约束

PPX 是面向日常办公人员的**本地优先桌面工具箱**：把图片、文档、表格、音视频、文件整理与自动化中高频的“输入—处理—检查结果—继续处理”收进同一工作台。它不以堆叠孤立小工具为目标，而以可追溯的本地处理链路减少文件来回选择、重复配置和结果丢失。

本阶段（2.10.0）在结果接力基础上完成表格处理链路：上一步的单份工作簿可进入质检、清洗或拆分，用户在明确的主输入区检查读取规则和样本，再处理完整数据。实现复用已有任务、操作、输出和工作区能力，不新增另一套流程引擎。
...
- 从失败明细构造新队列等增强接力尚未实现：本阶段优先保证单表处理正确性，避免把不同字段模型强行合并。
...
验证证据与范围见 [v2.10.0 发布说明](releases/v2.10.0.md)；上一版基线见 [v2.9.0 发布说明](releases/v2.9.0.md)。缺失依赖和原生安装场景单独保留，不并入通过项。
