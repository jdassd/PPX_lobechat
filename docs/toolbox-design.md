# PPX 统一工具箱设计方案（2.11.0 阶段）

## 1. 定位、目标与设计约束

PPX 是面向日常办公人员的**本地优先桌面工具箱**：把图片、文档、表格、音视频、文件整理与自动化中高频的“输入—处理—检查结果—继续处理”收进同一工作台。它不以堆叠孤立小工具为目标，而以可追溯的本地处理链路减少文件来回选择、重复配置和结果丢失。

本阶段（2.11.0）为已接通的工具链增加直接文件来源：用户可以检查某次处理使用了哪个任务的哪个输出，改选文件时来源随输入更新，重试与历史清理保留明确关系。表格处理继续沿用 2.10 的显式读取规则与类型一致变换。实现复用已有任务、操作、输出和工作区能力，不新增另一套流程引擎。
...
| P1 | 已确认链路可复制为工作流模板、完整来源索引 | 直接来源已支持，下一阶段优先让重复链路可复用，仍由用户确认运行 | 中等偏高；需明确参数复用和输出引用契约 |
...
- 保存为工作流模板、内容级来源校验、辅助文件来源、可解释推荐和跨模块参数映射尚未实现，按高频价值继续迭代。
- 旧的跨平台真实安装、升级、恢复、异常退出和逐功能真实数据验收仍为 pending。已有源码/构建与关键链路验证不能替代 Windows、macOS、Linux 的安装后人工验收，也不能覆盖真实登录网站与所有办公/媒体文件变体。

当前来源验证见 [v2.11.0 发布说明](releases/v2.11.0.md)；表格与接力基线见 [v2.10.0](releases/v2.10.0.md) 和 [v2.9.0](releases/v2.9.0.md)。缺失依赖和原生安装场景单独保留，不并入通过项。
