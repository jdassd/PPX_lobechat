# PPX 统一工具箱设计方案（2.16.0 阶段）

## 1. 定位、目标与设计约束

PPX 是面向日常办公人员的**本地优先桌面工具箱**：把图片、文档、表格、音视频、文件整理与自动化中高频的“输入—处理—检查结果—继续处理”收进同一工作台。它不以堆叠孤立小工具为目标，而以可追溯的本地处理链路减少文件来回选择、重复配置和结果丢失。

本阶段（2.16.0）完成重复候选审查与报告链路：按大小筛选后比较内容，明确同名、硬链接和不完整扫描的含义，将结果保留为可继续处理的 Excel 核对报告。复用已有有界搜索、预检、任务、字段选择、输出和工作流引擎，不增加运行依赖。
...
| 文件批处理 | 目录、筛选条件、规则、压缩项与可选共同根目录 | 有界搜索、重复审查、Excel 核对报告、结果接力、整理、压缩解压 | 搜索或比较不完整时保留已确认结果及原因；同名候选不代表内容重复；归档同名或路径冲突拒绝发布，保留源文件 | 已接力：任意文件打包归档；搜索来源与核对报告可进入兼容路由；可保留相对目录结构 |
| 自动化工作流 | 内置模板、已校验动作、参数、前序结果字段、触发器 | 当前参数保存后执行；顺序串联、部分成功策略、失败续跑、按需数据预览与文件接力 | 搜索归档和重复报告在不完整扫描后停止并排除输出目录；恢复要求签名与结果有效；查询源文件仍可触发监听 | 复用 operation 与 task；7 个内置模板，不接收任意脚本 |
...
| 文本去重 → 统一格式 | 多行文本、大小写或命名转换方式 | 文本逐行处理结果 → 文本格式转换；运行记录保留文本，不伪造文件产物 | 任一步错误停止；可查看或复制完整结果 |
| 搜索文档 → 保留目录归档 | 来源目录、独立输出目录、关键字、扩展名、递归、上限、包名 | `file_search.outputPaths` → `file_compress.items`，共同根目录保留相对路径 | 搜索不完整停止，空匹配不归档；路径冲突拒绝输出 |
| 重复文件扫描 → 核对报告 | 来源目录、独立报告目录、扩展名、递归、上限；默认比较内容，模式可在步骤内调整 | `file_deduplicate.groups/summary` → `file_deduplicate_report`；Excel 记录范围与逐文件明细 | 不完整扫描停止；完整空结果仍可生成核对报告 |
...
### 4.7 结果字段与文本处理（2.14.0）

1. 操作目录声明 `resultFields: [{path, label, type, optional?, when?}]`。仅前序步骤参与选择，按输入类型筛选；文件列表、第一份文件和结果目录来自现有规范化结果，目录不冒充单份文件。
2. 文本操作按模式声明字段：JSON 格式化/压缩输出文本，校验输出开关，查询输出动态类型；逐行处理输出文本或词频列表，并给出统计。绑定采用既有 `{{steps.id.field}}` 语法，不新增表达式执行器。
3. 未知运行参数保留“动态类型”标识；可选字段说明可能缺失。选择器帮助配置，但不保证文件数、结构化元素形状或运行时动态类型；预检和业务执行仍分别核对。
4. “文本去重 → 统一格式”接收多行文本及转换方式，先去首尾空白/空行并去重，再统一大小写或命名格式；错误停止后续步骤。结果保存在既有运行记录中，不生成虚假的文件资产。
5. 运行记录按步骤折叠数据结果，仅打开时生成可选择文本预览；上限 12000 个字符，复制保留完整值。`0`、`false`、空字符串和 `null` 分别呈现，不视为无结果。复制失败提示手动选择预览文本。
6. JSON 替换规则保留用户输入与光标；非法编辑也进入最新参数并被检查阻断，不静默使用旧规则。查询路径使用文本控件，转换方式和处理模式使用已支持的选项。

### 4.8 完整搜索与相对路径归档（2.15.0）

1. 搜索采用统一的本地目录扫描：文件名按字面包含、不区分大小写，扩展名/大小/修改时间在数量限制之前筛选；不依赖是否安装 fd，不应用 `.gitignore`，不跟随子链接或 Windows 联接点，跳过 PPX 历史和回收目录。
2. `matchedCount` 是已返回数量，不冒充总数。最多结果为 50–2000；只有发现第 `limit + 1` 个符合条件文件才标记截断。恰好达到上限且遍历完成仍算完整。
3. 限制扫描 50 万条目、128 层、30 秒，按目录和条目检查取消。读取异常、扫描预算不足保留有效匹配，同时 `complete=false/partial=true`；错误明细最多 20 项，总数独立累计。慢文件系统调用之间检查时限，不承诺中断正在进行的系统调用。
4. 手动搜索显示完整性、条目数和未完成原因；改筛选条件或失败时清除旧结果。选择兼容文件并发送后，接收面板仍需显式执行。
5. “搜索文档 → 保留目录归档”接收来源目录、输出目录、关键字、扩展名、递归开关、上限和包名；排除所选输出目录，完整匹配后才进入 ZIP 归档。空匹配不会产生归档，搜索不完整时停止并保留原因。
6. `file_compress.baseDir` 为可选共同根目录，ZIP/7Z 将来源的相对路径写入归档；启用后必须明确输出目录。根目录之外的输入、同名成员（包括仅大小写不同）、文件与目录冲突或包名夹带目录被拒绝，临时产物被清理，不覆盖已有文件。空目录与重复输入得到明确处理。
7. 查询返回的文件资产只表示可继续使用的来源。后端共享查询操作名单，监听只排除实际生成文件；对留存历史能确认的旧查询源标记进行一次修正，同时保留真实生成或无法判定的路径，不读取来源文件内容。

### 4.9 重复文件审查与核对报告（2.16.0）

1. 面向归档整理人员，输入扫描目录、扩展名、递归范围、比较方式与 100–20000 文件上限。复用有界目录扫描，额外匹配才判定截断；扫描进入既有任务队列，支持任务中心查看与取消。
2. 内容模式先按字节数筛选，再对同大小候选计算 SHA-256。读取前后核对身份、大小和修改时间；变化或不可读的文件不进入确认分组，并令整个扫描不完整。内容阶段有 60 秒协作预算；唯一大小文件无需读取内容。
3. 同名模式仅按不区分大小写的文件名分组，明确不比较内容、不计算重复体积。内容模式按可识别的独立文件身份估算多余逻辑字节；硬链接别名不重复计数，这一数字不承诺实际磁盘释放量。
4. 结果输出 `groups`、`summary`、`complete/partial` 与查询资产。摘要只存时间、编号、范围及计数，不重复嵌套全部路径。界面每次仅显示一个分组，每组每页 20 份文件；改变条件清除旧结果，迟到响应不能覆盖新条件。
5. `file_deduplicate_report` 接收分组、摘要和输出目录，将扫描时快照导出为 XLSX；说明页保存完整性、范围、同名/内容语义，明细页逐文件列出路径、大小和摘要。不重新读取来源文件，不自动删除；文本单元格强制为文本，避免公式样式名称被执行；上限 20000 文件，冲突自动避让。
6. 第七个模板“重复文件扫描 → 核对报告”将 `steps.scan.groups`、`steps.scan.summary` 交给报告动作；自动排除输出目录，不完整则停止。手动审查时允许显式导出不完整快照，报告必须标明不完整；无重复的完整扫描也可输出一份空明细核对记录。
7. 报告为真实生成资产，继续使用已有 Excel/索引/归档接力；扫描中的来源资产不加入监听生成文件名单。没有新增删除入口、全盘索引服务或第二套任务引擎。
8. 用户自行选择部分成功后继续时，续跑不能复用依赖旧扫描的报告。按实际绑定关系逐步传播重跑状态：依赖步骤重新生成，不混入旧产物；无关成功步骤仍沿用。查询重跑只返回本次匹配，历史显示重新生成的原因。
...
| P0（2.14.0） | 声明结果字段、类型筛选、文本模板和按需数据预览 | 将纯文本与结构化结果纳入统一工作流，消除输出字段猜测和结果不可见 | 中等；共享操作目录、既有历史与绑定语法，无新依赖 |
| P0（2.15.0） | 搜索完整性、相对路径归档、搜索接力和查询源监听 | 避免不完整或同名冲突的归档，复用已有文件结果与工作流 | 中等；统一有界扫描器和归档成员命名，以真实 ZIP/7Z、读取失败和监听事件验证 |
| P0（2.16.0） | 有界重复审查、同名/内容语义、核对报告和第七个模板 | 修正误导性空间指标和上限失效；将检查结果接入既有表格与归档工具 | 中等；复用扫描器、队列与 openpyxl；用真实硬链接、变化文件、工作簿和浏览器链路验证 |
...
```text
operation: { id, tool, feature, fields, resultFields, batchKey, dependencies, inputTypes, outputTypes }
...
- 后端已有版本化操作字段目录、操作参数校验、批量逐项执行、统一 `outputAssets` 补齐和持久化任务队列；本轮增补独立的重复报告操作。
...
- 已声明并验证常用文本、文件、质检结果字段；前序字段选择、第五个文本模板、按需数据预览与完整复制已实现。
- 第六个搜索归档模板、搜索状态/上限/范围、结果手动接力、ZIP/7Z 相对路径和碰撞阻断、查询来源与真实生成文件区分已实现。
- 第七个重复扫描报告模板、按大小筛选的可取消内容比较、同名候选语义、硬链接估算、分页审查与 Excel 快照报告已实现。
...
重复审查与报告验证见 [v2.16.0](releases/v2.16.0.md)，搜索归档见 [v2.15.0](releases/v2.15.0.md)，字段与文本链路见 [v2.14.0](releases/v2.14.0.md)，工作流检查与恢复见 [v2.13.0](releases/v2.13.0.md)，模板见 [v2.12.0](releases/v2.12.0.md)，来源见 [v2.11.0](releases/v2.11.0.md)；表格与接力基线见 [v2.10.0](releases/v2.10.0.md) 和 [v2.9.0](releases/v2.9.0.md)。缺失依赖和原生安装场景单独保留，不并入通过项。
