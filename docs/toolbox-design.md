# PPX 统一工具箱设计方案（2.14.0 阶段）

## 1. 定位、目标与设计约束

PPX 是面向日常办公人员的**本地优先桌面工具箱**：把图片、文档、表格、音视频、文件整理与自动化中高频的“输入—处理—检查结果—继续处理”收进同一工作台。它不以堆叠孤立小工具为目标，而以可追溯的本地处理链路减少文件来回选择、重复配置和结果丢失。

本阶段（2.14.0）补齐工作流数据链路：前序结果按实际声明字段和当前操作模式选择，运行后按需查看、复制文本与结构化结果。新增文本去重统一格式模板，并让替换规则、转换方式和 JSON 查询参数与实际执行契约一致。继续复用既有预检、任务、操作、输出和工作流引擎，不增加运行依赖。
...
| 自动化工作流 | 内置模板、已校验动作、参数、前序结果字段、触发器 | 当前参数保存后执行；顺序串联、部分成功策略、失败续跑、按需数据预览与文件接力 | 字段类型过滤，动态或可选字段在实际执行后核对；恢复要求签名与成果有效 | 复用 operation 与 task；5 个内置模板，不接收任意脚本 |
...
当前字段与文本链路验证见 [v2.14.0](releases/v2.14.0.md)，工作流检查与恢复见 [v2.13.0](releases/v2.13.0.md)，模板见 [v2.12.0](releases/v2.12.0.md)，来源见 [v2.11.0](releases/v2.11.0.md)；表格与接力基线见 [v2.10.0](releases/v2.10.0.md) 和 [v2.9.0](releases/v2.9.0.md)。缺失依赖和原生安装场景单独保留，不并入通过项。
