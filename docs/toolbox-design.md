# PPX 统一工具箱设计方案（2.9.0 阶段）

## 1. 定位、目标与设计约束

PPX 是面向日常办公人员的**本地优先桌面工具箱**：把图片、文档、表格、音视频、文件整理与自动化中高频的“输入—处理—检查结果—继续处理”收进同一工作台。它不以堆叠孤立小工具为目标，而以可追溯的本地处理链路减少文件来回选择、重复配置和结果丢失。

本阶段（2.9.0）的重点是“智能结果接力”：用户完成一次处理后，能在结果中看见适合的下一步，带着已确认的输出进入对应主输入区；接力不会替用户自动执行。实现复用已有任务、操作、输出和工作区能力，不新增另一套流程引擎。
...
| Excel 工具 | 单表、多个工作表/文件、表头行、字段映射、公式策略 | 预览、列画像、清洗、分组拆分、合并与质量报告 | 表头/映射不匹配、缓存公式不可用、写入失败要定位到字段或文件 | 已接力：多个 XLSX/XLSM/XLTX/XLTM 合并；单表清洗、质检的直接接力后续补齐 |
...
| 自动化工作流 | 已校验动作、参数、前序输出引用、触发器 | 白名单步骤串联、重试/退避、历史导出、监听与定时 | 失败续跑仅复用仍存在且签名一致的成功步骤；触发器排除自身输出 | 复用 operation 与 task；不接收未验证任意脚本 |
| 网页数据采集 | 用户点选字段、翻页规则、最大页数/条数、结果编号 | 采集记录与 XLSX/DOCX 导出 | 登录失效、动态页面、详情失败、限额需明确显示；只重试失败请求 | Playwright；导出→Excel 质检/清洗为规划链路 |
...
3. **表格 OCR → Excel 清洗/合并**：多个 XLSX 输出可直接接力到“合并 Excel”，预览字段后调整映射再运行；只有一份工作簿或输出为 CSV/JSON 时，先通过转换中心或人工选择进入清洗/质检。单表质检/清洗的直接接力列为 P1。
4. **采集导出 → 质量检查**：采集可导出 XLSX/DOCX，Excel 可导出质量报告；“导出后交给质检”是高价值规划链路，需在 Excel 主输入明确支持后再开放。

### 4.2 接力交互与数据规则

1. 结果弹窗先列出输出资产及不可发送项。目录、`exists:false`、空路径结果不可发送，并直接显示原因。
2. 操作列表随所选文件实时变化；模块关闭或依赖已知不可用的兼容操作禁用并显示原因，未知依赖明确标为尚未确认，支持重新检测。
3. 路由按真实路径扩展名和数量约束过滤：Windows 路径大小写不敏感去重，Linux 保持大小写；未知格式只建议归档；PDF 压缩、PDF OCR、视频压缩/截取为单文件。多文件不建议这些操作，直接调用也不能静默取第一项。
...
本阶段已接通路由表中的 **15 个目标操作**：图片压缩/水印/旋转、转换中心的通用转换/图片合 PDF/合并 PDF、PDF 压缩/OCR/合并、Word 合并、Excel 合并、文档索引、文件归档、视频压缩/截取。实现范围与验证环境分别记录在 [v2.9.0 发布说明](releases/v2.9.0.md)。

## 5. 优先级、可行性与成本

| 优先级 | 范围 | 价值与实现策略 | 复杂度/维护成本 |
| --- | --- | --- | --- |
| P0（2.9.0 当前阶段） | 结果资产规范化、兼容路由、结果选择与跳过说明、主输入精确消费、草稿保留 | 直接消除“处理完再找文件”的高频摩擦；复用 `operation_catalog`、`operations.py`、任务中心、`workspace.js` 和各面板既有提交逻辑 | 中等；重点维护路由与面板输入契约的一致性 |
| P0 | 补齐并验证路由表定义的 15 条接力链路 | 覆盖图片连续处理、转换、PDF、Word、Excel、索引、归档和视频等明确文件语义；每条链路均不自动运行 | 中等；需验证关闭模块、无依赖、混合文件、取消导航及主输入消费 |
| P1 | 基于已稳定的 15 条链路扩展表格 OCR→Excel、采集→质检等新路由 | 仅在接收面板主输入和输出类型明确后新增，避免以业务猜测替代兼容规则 | 中等；要逐模块检查依赖和多文件语义 |
| P1 | 接力来源与任务关联可视化、可复制为工作流模板 | 让重复链路可追溯、可复用，仍由用户确认运行 | 中等偏高；需定义运行历史引用的保留期 |
...
task:      { id, method, status, inputItems, itemResults, outputs, diagnosis }
asset:     { path, name, kind: file|directory, exists, size }
handoff:   { routeId, sourceTaskId?, sourceOperation?, assets, skipped, acceptedAt }
```

建议将 `sourceTaskId`、`sourceOperation`、`acceptedAt` 作为可选接力元数据加入前端状态或后端历史，而非把完整敏感参数复制进接力记录。`routeId` 必须稳定为 `tool/feature`；接收面板声明 `primaryInput`、可接受扩展名、最小/最大文件数及依赖要求。对目录、预览数据、纯文本结果和不存在路径，保持不作为文件接力资产的规则。
...
结果接力服务于临时、可审阅的两三步人工链路；工作流服务于稳定、重复、可配置触发的链路。用户从接力完成一次并确认参数稳定后，才适合将操作序列保存为工作流模板。工作流继续使用白名单操作、前序输出引用、步骤签名检查、失败续跑和触发器自身输出排除，不从前端接力队列直接绕过校验。
...
1. 在本阶段 15 条链路稳定后，再将表格 OCR→Excel、采集→质检等高频链路按接收能力逐条开放，并把“不可用原因”统一为模块状态提示。
2. 在任务详情展示来源与去向，支持将已确认的人工链路另存为工作流模板。
...
- 本轮已新增并接通 15 个接力目标：过滤目录、已知缺失、空路径、重复路径、不支持类型和超出单文件限制的输入；混合结果明确显示发送范围。
- 已移除文件对话框全局劫持，接收只发生在声明路由的主输入；辅助输入、其它模块和其它操作不消费队列。接收与执行分开。
- 已移除启动时的 Google Fonts 远程请求，使用本机字体回退，避免离线或慢网环境等待远程字体；页面语言标识设为简体中文。

### 未完成及原因

- 单表质检/清洗、从失败明细构造新队列等增强接力尚未实现：本阶段优先完成高频文件流转及统一接收边界，避免把不同字段模型强行合并。
- 接力来源追踪、保存为工作流模板、可解释推荐和跨模块参数映射尚未实现，避免在基础输入契约未稳定时引入高维护成本。
- 旧的跨平台真实安装、升级、恢复、异常退出和逐功能真实数据验收仍为 pending。已有源码/构建与关键链路验证不能替代 Windows、macOS、Linux 的安装后人工验收，也不能覆盖真实登录网站与所有办公/媒体文件变体。

本阶段验证结果见 [v2.9.0 发布说明](releases/v2.9.0.md)：本机已通过 14 个目标接收及 OCR 缺失依赖禁用检查、混合/重复/缺失结果处理、辅助输入隔离、不自动执行、短窗口明暗主题、草稿保留、静态检查、生产构建与原有核心流程。未运行的 OCR 接收/识别及原生安装场景单独保留，不并入通过项。
