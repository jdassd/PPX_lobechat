# PPX 统一工具箱设计方案（2.12.0 阶段）

## 1. 定位、目标与设计约束

PPX 是面向日常办公人员的**本地优先桌面工具箱**：把图片、文档、表格、音视频、文件整理与自动化中高频的“输入—处理—检查结果—继续处理”收进同一工作台。它不以堆叠孤立小工具为目标，而以可追溯的本地处理链路减少文件来回选择、重复配置和结果丢失。

本阶段（2.12.0）让重复的图片交付和表格检查能够通过常用模板直接复用，并保证执行当前参数、部分成功时明确停止和失败后可靠续跑。继续保留 2.11 的直接文件来源及 2.10 的类型一致表格处理；实现复用已有任务、操作、输出、工作区和工作流引擎。
...
当前工作流验证见 [v2.12.0 发布说明](releases/v2.12.0.md)，来源见 [v2.11.0](releases/v2.11.0.md)；表格与接力基线见 [v2.10.0](releases/v2.10.0.md) 和 [v2.9.0](releases/v2.9.0.md)。缺失依赖和原生安装场景单独保留，不并入通过项。
